...
from classes.contrary import Contrary
...
class ABAFramework:
    ...
    def plot_aba_graph(self, output_html="aba_graph.html"):
        ...
        output_html = "./output/"+output_html
    ...
    def plot_aba_plus_graph(self, output_html="aba_plus_graph.html"):
        ...
        for S in self.assumption_combinations:
            label = "{" + ",".join(str(l)
                                   for l in sorted(S, key=str)) + "}" if S else "{}"
            net.add_node(str(frozenset(S)), label=label)

        # Normal attacks
        for att in self.normal_attacks:
            net.add_edge(str(att[0]), str(att[1]), color='black',
                         dashes=False, width=2, title="Normal Attack")

        # Reverse attacks
        for att in self.reverse_attacks:
            net.add_edge(str(att[0]), str(att[1]), color='red',
                         dashes=True, width=2, title="Reverse Attack")
        output_html = "./output/"+output_html
        net.write_html(output_html)
